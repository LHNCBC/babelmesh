    ...
    <div class="usa-layout padding-y-4">
      <div class="grid-row grid-gap">
        ...
        <div class="desktop:grid-col-9">
          <p>BABELMESH</p>
          <h2>Cerca in MEDLINE/PubMed in Italiano</h2>
...
<font size="2">
In fase di ricerca, è forse più semplice usare la lingua che si conosce meglio. La traduzione dei "<a href=http://www.nlm.nih.gov/pubs/factsheets/mesh.html >MeSH</a>" può essere utile a chi non conosce bene l'inglese.
</font>
<p>
<b>Inserire un termine medico o una frase in italiano</b></br>
</p>
...
<form  accept-charset="utf-8" method="POST" name ="ita" id="query" action=<?php echo $_SERVER['PHP_SELF']."?com=$com"; ?>>
  ...
  <!--<input type="submit" value="Invia" name="B1">-->
	...
	<font size="2" color="#0000FF">Mostra gli articoli pubblicati in:<br></font>
<font size="2" color="#0000FF">[<span id="s_ara_o">Arabo</span><input type="checkbox" value="ARA" name="ARA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_chn_o">Cinese</span><input type="checkbox" value="CHN" name="CHN_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_fre_o">Francese</span><input type="checkbox" value="FRE" name="FRE_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ger_o">Tedesco</span><input type="checkbox" value="GER" name="GER_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ita_o">Italiano</span><input type="checkbox" value="ITA" name="ITA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_jpn_o">Giapponese</span><input type="checkbox" value="JPN" name="JPN_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_kor_o">Coreano</span><input type="checkbox" value="KOR" name="KOR_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_por_o">Portoghese</span><input type="checkbox" value="POR" name="POR_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_rus_o">Russo</span><input type="checkbox" value="RUS" name="RUS_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_spa_o">Spagnolo</span><input type="checkbox" value="SPA" name="SPA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_swe_o">Svedese</span><input type="checkbox" value="SWE" name="SWE_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_eng_o">Inglese</span><input type="checkbox" value="ENG" name="ENG_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_all_o">Tutto</span><input type="checkbox" value="ALL" name="ALL_o">]</font>

	<input type = "hidden" value="1" name="checked">
</form>

<SCRIPT src="alllatin.js"></SCRIPT>
<SCRIPT>InstallAC(document.ita,document.ita.terms,document.ita.B1,"search","en", "ita");</SCRIPT>
<?php
	}
  elseif ($_POST["checked"] == 1){
			...
			//$_POST["terms"] = $dele->clean_ita($_POST["terms"]);
			unset($dele);
			
  		$db_name = 'crosslan';
  		$db_object = db_connect($db_name);

			$query = "SELECT US_DESCRIPTOR FROM ita_mesh WHERE ita_descriptor ='".addslashes($_POST["terms"])."'";
      ...
      $ip = get_client_ip();
	  	if ($checked == 1) {
				 ...
				 $content .= "Italian: ".$_POST["terms"]." -- ".$rterm;
...
<form enctype="application/x-www-form-urlencoded" name="query2" action="search.php?from=ita&com=<?php echo $com;?>&term=<?php echo $term; ?>" method="POST">
...
<?php
			...
			elseif ($checked == 2) {
				 $content = $ip." - [".date("j/M/Y: G:i:s")."] ";
				 $content .= "Italian: ".$_POST["terms"]." -- ???";
         $query = "INSERT INTO log (search) VALUES ('$content')";
         mysqli_query($db_object, $query) or die("Insert query failed : ");
				 
				 $find= new translate();
				 $de_chars = array("?", ".", ",", ";", ":");
				 $result = $find->I_no_order(addslashes(str_replace($de_chars, " ", $_POST["terms"]) ));
				 if ($result == '') {
				   $result = $find->Ifinder(addslashes(str_replace($de_chars, " ", $_POST["terms"]) ));
					 $FINDER = 0;
					 if(sizeof($result) != 0) {
					   $FINDER = 1;
					 } else {
				 	   $query = "SELECT ITA_DESCRIPTOR, US_DESCRIPTOR FROM ita_mesh WHERE ITA_DESCRIPTOR like '%".addslashes($_POST["terms"])."%' GROUP BY US_DESCRIPTOR";
	  		 		 $result = mysqli_query($db_object,$query); //or die("Query failed : " );
				 		 if (mysqli_affected_rows($db_object) == 0) {
							   $result=$find->topI(addslashes($_POST["terms"]));
							 	 if (mysqli_affected_rows($db_object)==0) {
							  	 echo "Questo termine non e' nel database.<br>";
									 echo "<p><form action=search.php?from=ita&com=$com method=POST>".$_POST["terms"]." = <input value=\"".$_POST["terms"]."\" size=25 name=terms> ";
									 echo "<input type=submit value=Clicca>";
									 echo "<input type = \"hidden\" name=\"Oterm\" value=\"".$_POST["terms"]."\">";
									 echo "</form><p>";

									 echo "E' possibile proseguire la ricerca utilizzando termini in inglese. (Although it's not in our database, you may modify this search by adding or deleting English terms.)<hr>";
									 echo "<p><a href=".$_SERVER['PHP_SELF'].">Ritorna alla ricerca iniziale</a>";
									 //die();							
                   goto end;
							 	 }			
				 		 }
				 ...
				 echo "Forse cercavi?<p>";
				 echo "<font color=\"#3333CC\">E' possibile modificare questa ricerca inserendo termini in inglese:</font><p>";
				 if ($FINDER == 1) {
				   $term = implode(" ", $result);		 
					 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=ita&com=$com\" method=\"POST\">";	
					 $temp = $_POST["terms"]; //$find->clean_ita($_POST["terms"]);
					 echo "<li>".$temp;
					 echo "&nbsp;=&nbsp;<input name=\"terms\" size=\"25\" value=\"".$term."\">";
					 echo "<input type = \"hidden\" name=\"Oterm\" value=\"".$temp."\">";
					 echo "&nbsp;<input type=\"submit\" value=\"Clicca\" name=\"B1\">";
				   ...
				   while ($line= mysqli_fetch_array($result)) {	
				 	   $term = $line["US_DESCRIPTOR"];
					 	 $term1 = $term;
						 $term = str_replace(' ', '+',$term);
						 $temp = $line["ITA_DESCRIPTOR"];
						 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=ita&com=$com\" method=\"POST\">";
					 	 echo "<li>".str_replace(strtolower($_POST["terms"]), "<B style=\"color:black;background-color:#ffff66\">".strtolower($_POST["terms"])."</B>", strtolower($line["ITA_DESCRIPTOR"]));
