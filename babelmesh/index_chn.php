...
<font size="2">
在信息搜索中，使用熟悉的语言会更为简便。“<a href=http://www.nlm.nih.gov/pubs/factsheets/mesh.html >MeSH</a>"的翻译适合于不熟悉英语的用户。
</font>
<p>
<b>用中文输入医学词汇：</b></br>
</p>
...
<form accept-charset="utf-8" method="POST" name ="chn" id="query" action=<?php echo $_SERVER['PHP_SELF']."?com=$com"; ?>>
  ...
  <button class="g-recaptcha" 
        ...
        data-action='submit'>搜索</button>
  <!--<input type="submit" value="搜索" name="B1">-->
	&nbsp;<input type="reset" value="取消" name="B2"></p>
	<font size="2" color="#0000FF">原著文章语言选择:<br></font>
<font size="2" color="#0000FF">[<span id="s_ara_o">阿拉伯语</span><input type="checkbox" value="ARA" name="ARA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_chn_o">中文</span><input type="checkbox" value="CHN" name="CHN_o" >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_fre_o">法语</span><input type="checkbox" value="FRE" name="FRE_o" >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ger_o">德语</span><input type="checkbox" value="GER" name="GER_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ita_o">意大利语</span><input type="checkbox" value="ITA" name="ITA_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_jpn_o">日语</span><input type="checkbox" value="JPN" name="JPN_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_kor_o">韩语</span><input type="checkbox" value="KOR" name="KOR_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_por_o">葡萄牙语</span><input type="checkbox" value="POR" name="POR_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_rus_o">俄语</span><input type="checkbox" value="RUS" name="RUS_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_spa_o">西班牙语</span><input type="checkbox" value="SPA" name="SPA_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_swe_o">瑞典语</span><input type="checkbox" value="SWE" name="SWE_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_eng_o">英语</span><input type="checkbox" value="ENG" name="ENG_o" >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_all_o">所有语言</span><input type="checkbox" value="ALL" name="ALL_o">]</font>

	<input type = "hidden" value="1" name="checked">
</form>

<SCRIPT src="alllang.js"></SCRIPT>
<SCRIPT>InstallAC(document.chn,document.chn.terms,document.chn.B1,"search","en", "chn");</SCRIPT>
<?php
	}
  elseif ($_POST["checked"] == 1){
		$ooterm = $_POST["terms"];
  		...
  		$db_object = db_connect($db_name);
			...
			$query = "SELECT US_DESCRIPTOR FROM chn_mesh WHERE CHN_DESCRIPTOR ='".addslashes($_POST["terms"])."'";
      ...
      $ip = get_client_ip();
	  	if ($checked == 1) {
				 ...
				 $content .= "Chinese: ".$_POST["terms"]." -- ".$rterm;
...
<form enctype="application/x-www-form-urlencoded" name="query2" action="search.php?from=chn&com=<?php echo $com;?>&term=<?php echo $term; ?>" method="POST">
...
<?php
         ...
         mysqli_query($db_object, $query) or die("Insert query failed : ");
				   ...
				   $result = $find->lang_finder(addslashes(str_replace($de_chars, " ", $_POST["terms"]) ), $lang, $lang_table, $lang_column, $eng_column);
					 ...
					 if(sizeof($result) != 0) {
					   $FINDER = 1;
				 	   ...
				 	   $query = "SELECT CHN_DESCRIPTOR, US_DESCRIPTOR FROM chn_mesh WHERE CHN_DESCRIPTOR like '%".addslashes($_POST["terms"])."%' GROUP BY US_DESCRIPTOR";
						 $query = $query." ORDER BY CUI DESC";
	  		 		 $result = mysqli_query($db_object,$query); //or die("Query failed : " );
				 		 if (mysqli_affected_rows($db_object) == 0) {
							   $result=$find->topLang(addslashes($_POST["terms"]), $lang, $lang_table, $lang_column, $eng_column);
							 	 if (mysqli_affected_rows($db_object)==0) {
							  	 echo "Sorry, this term is not in our database.<br>";
									 echo "<p><form action=search.php?from=chn&com=$com method=POST>".$ooterm." = <input value=\"".$ooterm."\" size=25 name=terms> ";
				 ...
				 if ($FINDER == 1) {
				   $term = implode(" ", $result);		 
					 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=chn&com=$com\" method=\"POST\">";	
				   ...
				   while ($line= mysqli_fetch_array($result)) {	
				 	   $term = $line["US_DESCRIPTOR"];
					 	 $term1 = $term;
						 $term = str_replace(' ', '+',$term);
						 $temp = $line["CHN_DESCRIPTOR"];
						 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=chn&com=$com\" method=\"POST\">";
						 echo "<li>".str_replace($ooterm, "<B style=\"color:black;background-color:#ffff66\">".$ooterm."</B>", mb_convert_encoding($line["CHN_DESCRIPTOR"], "CP1252"));
