...
<?php
function ABdisplay($id, $fterm) {
  global $from, $stopwords, $ncbi_key;
  ...
  unset($getxml); 
	...
	$termarray = explode(" ", str_replace( array('(', ')', 'OR', 'AND'), "", trim(urldecode($fterm))));
  $titemp = $ti;
	$ii = 0;
  ...
  else
  {
  	...
  	for ($i=0; $i<sizeof($termarray); $i++) {
	    $testterm = ' '.strtolower($termarray[$i]).' ';
			...
			if (in_array(strtolower($termarray[$i]), $stopwords) == false) { // begin non-stop word
	  	if (($ii%5)==0) {
	      $abtemp = preg_replace('/'.$testterm.'/i', " <B style=\"color:black;background-color:#ffff66\">".strtolower($termarray[$i])."</B> ", $newtemp);
			}
	  	elseif (($ii%5)==1) {
	      $abtemp = preg_replace('/'.$testterm.'/i', " <B style=\"color:black;background-color:#00FF00\">".strtolower($termarray[$i])."</B> ", $newtemp);
	  	}
			elseif (($ii%5)==2) {
	      $abtemp = preg_replace('/'.$testterm.'/i', " <B style=\"color:black;background-color:#3399FF\">".strtolower($termarray[$i])."</B> ", $newtemp);
			}
			elseif (($ii%5)==3){
		    $abtemp = preg_replace('/'.$testterm.'/i', " <B style=\"color:black;background-color:#CC9900\">".strtolower($termarray[$i])."</B> ", $newtemp);
			}
			else {
		    $abtemp = preg_replace('/'.$testterm.'/i', " <B style=\"color:black;background-color:#66CCFF\">".strtolower($termarray[$i])."</B> ", $newtemp);
			}
			$ii++;
			} // end non-stop word
		}
...
call_user_func('ABdisplay', $id, $fterm);
