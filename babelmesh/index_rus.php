    ...
    <div class="usa-layout padding-y-4">
      <div class="grid-row grid-gap">
        ...
        <div class="desktop:grid-col-9">
          <p>BABELMESH</p>
          <h2> Введите поиск в MEDLINE/PubMed на русском языке</h2>
...
<font size="2">
Для поиска информации удобнее всего пользоваться языком, который вам наиболее знаком. Переводы «<a href=http://www.nlm.nih.gov/pubs/factsheets/mesh.html >MeSH</a>» помогут лицам, мало владеющим английским языком, вести поиск в MEDLINE/PubMed употребляя медицинские термины и выражения. Полученные в результате поиска цитаты и рефераты будут только на английском языке.
</font>
<p>
<b>Введите медицинский термин или выражение на русском языке</b></br>
</p>
...
<form  accept-charset="utf-8" method="POST" name ="rus" id="query" action=<?php echo $_SERVER['PHP_SELF']."?com=$com"; ?>>
  ...
  <button class="g-recaptcha" 
        ...
        data-action='submit'>Поиск</button>
  <!--<input type="submit" value="Поиск" name="B1">-->
	&nbsp;<input type="reset" value="Удалить" name="B2"></p>

<font size="2" color="#0000FF">Укажите журнальные статьи изданные на следующем языке:<br></font>
<font size="2" color="#0000FF">[<span id="s_ara_o">Арабский</span><input type="checkbox" value="ARA" name="ARA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_chn_o">китайский</span><input type="checkbox" value="CHN" name="CHN_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_fre_o">Французский</span><input type="checkbox" value="FRE" name="FRE_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ger_o">Немецкий</span><input type="checkbox" value="GER" name="GER_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ita_o">Итальянский</span><input type="checkbox" value="ITA" name="ITA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_jpn_o">Японский</span><input type="checkbox" value="JPN" name="JPN_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_kor_o">Корейский</span><input type="checkbox" value="KOR" name="KOR_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_por_o">Португальский</span><input type="checkbox" value="POR" name="POR_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_rus_o">Русский</span><input type="checkbox" value="RUS" name="RUS_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_spa_o">Испанский</span><input type="checkbox" value="SPA" name="SPA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_swe_o">Шведский</span><input type="checkbox" value="SWE" name="SWE_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_eng_o">Английский</span><input type="checkbox" value="ENG" name="ENG_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_all_o">Все</span><input type="checkbox" value="ALL" name="ALL_o">]</font>

	<input type = "hidden" value="1" name="checked">
	
</form>

<SCRIPT src="alllang.js"></SCRIPT>
<SCRIPT>InstallAC(document.rus,document.rus.terms,document.rus.B1,"search","en","rus");</SCRIPT>
<?php
	}
  elseif ($_POST["checked"] == 1){
		...
		$ooterm = $_POST["terms"];
			...
			$_POST["terms"] = mb_convert_encoding($_POST["terms"],"UTF-8", "CP1252");
			$_POST["terms"] = str_replace("�","",$_POST["terms"]);
			
  		$db_name = 'crosslan';
  		$db_object = db_connect($db_name);

			$query = "SELECT US_DESCRIPTOR FROM rus_mesh WHERE rus_descriptor ='".addslashes($_POST["terms"])."'";
      ...
      $ip = get_client_ip();
	  	if ($checked == 1) {
				 ...
				 $content .= "Russian: ".$_POST["terms"]." -- ".$rterm;
...
<form enctype="application/x-www-form-urlencoded" name="query2" action="search.php?from=rus&com=<?php echo $com;?>&term=<?php echo $term; ?>" method="POST">
...
<?php
         ...
         mysqli_query($db_object, $query) or die("Insert query failed : ");
				   ...
				   $result = $find->lang_finder(addslashes(str_replace($de_chars, " ", $_POST["terms"]) ), $lang, $lang_table, $lang_column, $eng_column);
					 ...
					 if(sizeof($result) != 0) {
					   $FINDER = 1;
				 	   ...
				 	   $query = "SELECT RUS_DESCRIPTOR, US_DESCRIPTOR FROM rus_mesh WHERE RUS_DESCRIPTOR like '%".addslashes($_POST["terms"])."%' GROUP BY US_DESCRIPTOR";
	  		 		 $result = mysqli_query($db_object,$query); //or die("Query failed : " );
				 		 if (mysqli_affected_rows($db_object) == 0) {
							   $result=$find->topLang(addslashes($_POST["terms"]), $lang, $lang_table, $lang_column, $eng_column);
							 	 if (mysqli_affected_rows($db_object)==0) {
							  	 echo "Извините, слово или выражение в базе данных отсутствует.<br>";
									 echo "<p><form action=search.php?from=rus&com=$com method=POST>".$_POST["terms"]." = <input value=\"".$_POST["terms"]."\" size=25 name=terms> ";
									 echo "<input type=submit value=\"Поиск\">";
									 echo "<input type = \"hidden\" name=\"Oterm\" value=\"".$_POST["terms"]."\">";
									 echo "</form><p>";

									 echo "Несмотря на отсутствие термина или выражения в нашей базе данных, вы можете продолжать поиск подобных терминов на английском языке.";
				 ...
				 if ($FINDER == 1) {
				   $term = implode(" ", $result);		 
					 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=rus&com=$com\" method=\"POST\">";	
				   ...
				   while ($line= mysqli_fetch_array($result)) {	
				 	 ...
				 	 //$term = str_replace(',', '+',$term);
					 	 $term1 = $term;
						 $term = str_replace(' ', '+',$term);
						 $temp = $line["RUS_DESCRIPTOR"];
						 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=rus&com=$com\" method=\"POST\">";
						 echo "<li>".str_replace($ooterm, "<B style=\"color:black;background-color:#ffff66\">".$ooterm."</B>", mb_convert_encoding($line["RUS_DESCRIPTOR"], "CP1252"));
