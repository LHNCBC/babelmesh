...
<P><FONT size=2>BabelMeSH는 우리에게 친숙한 한글을 이용해서 영문논문을 검색할 수 있도록 해 줍니다. ("<A 
href="http://www.nlm.nih.gov/pubs/factsheets/mesh.html">MeSH</A>" 번역은 보건복지부 보건의료정보 표준화 위원회에서 제공한 자료를 사용했습니다)</FONT>
<P><B>한글 의학용어를 입력해 주십시오 :</B><BR></P>
...
<form  accept-charset="utf-8" method="POST" name ="kor" id="query" action=<?php echo $_SERVER['PHP_SELF']."?com=$com"; ?>>
  ...
  <button class="g-recaptcha" 
        ...
        data-action='submit'>찾기</button>
  <!--<input type="submit" value="찾기" name="B1">-->
	&nbsp;<input type="reset" value="지우기" name="B2"></p>
<FONT color=#0000ff size=2>검색에 포함할 논문의 언어:<BR>
[<SPAN 
id=s_ara_o>아랍어</SPAN><INPUT type=checkbox value=ARA 
name=ARA_o>
]&nbsp;&nbsp;&nbsp;</FONT> <FONT color=#0000ff size=2>[<SPAN 
id=s_chn_o>중국어</SPAN>
<INPUT type=checkbox value=CHN 
name=CHN_o>
]&nbsp;&nbsp;&nbsp;</FONT> <FONT color=#0000ff size=2>[<SPAN 
id=s_fre_o>프랑스어</SPAN>
<INPUT type=checkbox value=FRE 
name=FRE_o>
]&nbsp;&nbsp;&nbsp;</FONT> <FONT color=#0000ff size=2>[<SPAN 
id=s_ger_o>독일어</SPAN>
<INPUT type=checkbox value=GER 
name=GER_o>
]&nbsp;&nbsp;&nbsp;</FONT> <FONT color=#0000ff size=2>[<SPAN 
id=s_ita_o>한국어</SPAN>
...
<FONT color=#0000ff size=2>[<SPAN 
id=s_kor_o>이탈리아어</SPAN>
<INPUT type=checkbox value=KOR 
name=KOR_o>
]&nbsp;&nbsp;&nbsp;</FONT> <FONT color=#0000ff size=2>[<SPAN 
id=s_por_o>포루투갈어</SPAN>
<INPUT type=checkbox value=POR 
name=POR_o>
]&nbsp;&nbsp;&nbsp;</FONT> <FONT color=#0000ff size=2>[<SPAN 
id=s_rus_o>러시아어</SPAN>
<INPUT type=checkbox value=RUS 
name=RUS_o>
]&nbsp;&nbsp;&nbsp;</FONT> <FONT color=#0000ff size=2>[<SPAN 
id=s_spa_o>스페인어</SPAN>
<INPUT type=checkbox value=SPA 
name=SPA_o>
]&nbsp;&nbsp;&nbsp;</FONT> <FONT color=#0000ff size=2>[<SPAN 
id=s_swe_o>스웨덴 어의</SPAN>
<INPUT type=checkbox value=SWE 
name=SWE_o>
]&nbsp;&nbsp;&nbsp;</FONT> <FONT color=#0000ff size=2>[<SPAN 
id=s_eng_o>영어</SPAN>
...
id=s_all_o>모든 언어 </SPAN>
<INPUT type=checkbox value=ALL name=ALL_o>]</FONT> <INPUT 
type=hidden value=1 name=checked> </FORM>

<SCRIPT src="alllang.js"></SCRIPT>
<SCRIPT>InstallAC(document.kor,document.kor.terms,document.kor.B1,"search","en","kor");</SCRIPT>
  ...
  elseif ($_POST["checked"] == 1){
		...
		$ooterm = $_POST["terms"];
  		...
  		$db_object = db_connect($db_name);
			...
			$query = "SELECT US_DESCRIPTOR, KOR_DESCRIPTOR FROM kor_mesh WHERE kor_descriptor ='".addslashes($_POST["terms"])."'";
	  	$result = mysqli_query($db_object,$query) or die("Query failed : ");
			
			$my_rows = 0;
			$my_rows = mysqli_affected_rows($db_object);
			if ($my_rows == 0) {
			    $checked = 2;
			}			
			else {					
				   ...
				   echo "Do you mean?<p>";
					 ...
					 while ($line= mysqli_fetch_array($result)) {	
				 	   $term = $line["US_DESCRIPTOR"];
						 ...
						 $temp = $line["KOR_DESCRIPTOR"];
						 
						 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=kor&com=$com\" method=\"POST\">";
						 if ($temp == $_POST["terms"]) {
						   echo "<li>".$temp;
						 	 echo "&nbsp;=&nbsp;<input name=\"terms\" size=\"25\" value=\"".$term1."\">";
						 	 echo "<input type = \"hidden\" name=\"Oterm\" value=\"".$temp."\">";
						 	 echo "<input type = \"hidden\" name=\"logyn\" value=\"1\">";
				 ...
				 elseif ($my_rows == 1) {
				   $line= mysqli_fetch_array($result);
					 $term = $line["US_DESCRIPTOR"];	
				 	 ...
				 	 $content .= "Korean: ".$_POST["terms"]." -- ".$rterm;
...
<form enctype="application/x-www-form-urlencoded" name="query2" action="search.php?from=kor&com=<?php echo $com;?>&term=<?php echo $term; ?>" method="POST">
				   ...
				   $result = $find->lang_finder(addslashes(str_replace($de_chars, " ", $_POST["terms"]) ), $lang, $lang_table, $lang_column, $eng_column);
					 ...
					 if(sizeof($result) != 0) {
					   $FINDER = 1;
				 	   ...
				 	   $query = "SELECT KOR_DESCRIPTOR, US_DESCRIPTOR FROM kor_mesh WHERE KOR_DESCRIPTOR like '%".addslashes($_POST["terms"])."%' GROUP BY US_DESCRIPTOR";
	  		 		 $result = mysqli_query($db_object,$query); //or die("Query failed : " );
							   ...
							   $result=$find->topLang(addslashes($_POST["terms"]), $lang, $lang_table, $lang_column, $eng_column);
							 	 if (mysqli_affected_rows($db_object)==0) {
							  	 echo "Sorry, this term is not in our database.<br>";
									 echo "<p><form action=search.php?from=kor&com=$com method=POST>".$ooterm." = <input value=\"".$ooterm."\" size=25 name=terms> ";
				 ...
				 echo "find<br>";
				   ...
				   while ($line= mysqli_fetch_array($result)) {	
				 	   $term = $line["US_DESCRIPTOR"];
						 ...
						 $temp = $line["KOR_DESCRIPTOR"];
						 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=kor&com=$com\" method=\"POST\">";
						 echo "<li>".str_replace($ooterm, "<B style=\"color:black;background-color:#ffff66\">".$ooterm."</B>", mb_convert_encoding($line["KOR_DESCRIPTOR"], "CP1252"));
