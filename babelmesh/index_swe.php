    ...
    <div class="usa-layout padding-y-4">
      <div class="grid-row grid-gap">
        ...
        <div class="desktop:grid-col-9">
          <p>BABELMESH</p>
          <h2>Sök i MEDLINE/PubMed på svenska</h2>
...
<p>
<font size="2">
När man söker information, är det ofta enklast att använda det språk som man är van vid. Översättningar av <a href=http://www.nlm.nih.gov/pubs/factsheets/mesh.html >MeSH</a>-vokabulären kan underlätta om man inte behärskar engelsk medicinsk terminologi.
</font>
<p>
<b>Skriv in en medicinsk term på svenska</b></br>
</p>
...
<form  accept-charset="utf-8" method="POST" name ="swe" id="query" action=<?php echo $_SERVER['PHP_SELF']."?com=$com"; ?>>
  ...
  <button class="g-recaptcha" 
        ...
        data-action='submit'>"Sök"</button>
  <!--<input type="submit" value="Sök" name="B1">-->
	&nbsp;<input type="reset" value="Rensa" name="B2"></p>

<font size="2" color="#0000FF">Visa tidskriftsartiklar på:<br></font>
<font size="2" color="#0000FF">[<span id="s_ara_o">Arabiska</span><input type="checkbox" value="ARA" name="ARA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_chn_o">Kinesiska</span><input type="checkbox" value="CHN" name="CHN_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_fre_o">Franska</span><input type="checkbox" value="FRE" name="FRE_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ger_o">Tyska</span><input type="checkbox" value="GER" name="GER_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ita_o">Italienska</span><input type="checkbox" value="ITA" name="ITA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_jpn_o">Japanska</span><input type="checkbox" value="JPN" name="JPN_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_kor_o">Koreanska</span><input type="checkbox" value="KOR" name="KOR_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_por_o">Portugisiska</span><input type="checkbox" value="POR" name="POR_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_rus_o">Ryska</span><input type="checkbox" value="RUS" name="RUS_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_spa_o">Spanska</span><input type="checkbox" value="SPA" name="SPA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_swe_o">Svenska</span><input type="checkbox" value="SWE" name="SWE_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_eng_o">Engelska</span><input type="checkbox" value="ENG" name="ENG_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_all_o">Alla språk</span><input type="checkbox" value="ALL" name="ALL_o">]</font>

	<input type = "hidden" value="1" name="checked">
</form>

<SCRIPT src="alllatin.js"></SCRIPT>
<SCRIPT>InstallAC(document.swe,document.swe.terms,document.swe.B1,"search","en","swe");</SCRIPT>
<?php
	}
  elseif ($_POST["checked"] == 1){
			...
			$dele= new translate();
			$_POST["terms"] = $dele->lang_delete(utf8_encode($_POST["terms"]));
  		...
  		$db_object = db_connect($db_name);
			...
			$query = "SELECT US_DESCRIPTOR FROM swe_mesh WHERE swe_descriptor ='".addslashes($_POST["terms"])."'";
      ...
      $ip = get_client_ip();
	  	if ($checked == 1) {
				 ...
				 $content .= "Swedish: ".$_POST["terms"]." -- ".$rterm;
...
<form enctype="application/x-www-form-urlencoded" name="query2" action="search.php?from=swe&com=<?php echo $com;?>&term=<?php echo $term; ?>" method="POST">
...
<?php
         ...
         mysqli_query($db_object, $query) or die("Insert query failed : ");
				   ...
				   $result = $find->lang_finder(addslashes(str_replace($de_chars, " ", $_POST["terms"]) ), $lang, $lang_table, $lang_column, $eng_column);
					 ...
					 if(sizeof($result) != 0) {
					   $FINDER = 1;
				 	   ...
				 	   $query = "SELECT SWE_DESCRIPTOR, US_DESCRIPTOR FROM swe_mesh WHERE SWE_DESCRIPTOR like '%".addslashes($_POST["terms"])."%' GROUP BY US_DESCRIPTOR";
	  		 		 $result = mysqli_query($db_object,$query); //or die("Query failed : " );
				 		 if (mysqli_affected_rows($db_object) == 0) {
							   $result=$find->topLang(addslashes($_POST["terms"]), $lang, $lang_table, $lang_column, $eng_column);
							 	 if (mysqli_affected_rows($db_object)==0) {
							  	 echo "Tyvärr, denna term fanns inte i databasen.<br>";
									 echo "<p><form action=search.php?from=swe&com=$com method=POST>".$_POST["terms"]." = <input value=\"".$_POST["terms"]."\" size=25 name=terms> ";
									 echo "<input type=submit value=Search>";
									 echo "<input type = \"hidden\" name=\"Oterm\" value=\"".$_POST["terms"]."\">";
									 echo "</form><p>";

									 echo "Just denna term finns inte, men prova att ta bort eller lägga till engelska sökord och försök igen. <hr>";
									 echo "<p><a href=".$_SERVER['PHP_SELF'].">Tillbaka till söksidan</a>";
									 //die();							
                   goto end;
							 	 }			
				 		 }
				 ...
				 echo "Menade du detta?<p>";
				 echo "<font color=\"#3333CC\">Ta bort eller lägg till engelska sökord och försök igen:</font><p>";
				 if ($FINDER == 1) {
				   $term = implode(" ", $result);		 
					 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=swe&com=$com\" method=\"POST\">";	
				   ...
				   while ($line= mysqli_fetch_array($result)) {	
				 	   $term = $line["US_DESCRIPTOR"];
					 	 $term1 = $term;
						 $term = str_replace(' ', '+',$term);
						 $temp = $line["SWE_DESCRIPTOR"];
						 
						 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=swe&com=$com\" method=\"POST\">";
						 //echo "<br>".$_POST["terms"]."<br>";
					 	 echo "<li>".str_replace(strtolower($_POST["terms"]), "<B style=\"color:black;background-color:#ffff66\">".strtolower($_POST["terms"])."</B>", strtolower($line["SWE_DESCRIPTOR"]));
    ...
    <div class="row mt-3">
      ...
      <div class="col-md-3 col-sm-6 col-6">
        <p><a href="https://www.nlm.nih.gov/web_policies.html" class="text-white"> Web Policies </a><br>
          <a href="https://www.nih.gov/institutes-nih/nih-office-director/office-communications-public-liaison/freedom-information-act-office" class="text-white"> FOIA </a><br><a href="https://www.hhs.gov/vulnerability-disclosure-policy/index.html" class="text-white">Vulnerability Disclosure</a></p>
      </div>
      <div class="col-md-3 col-sm-6 col-6">
