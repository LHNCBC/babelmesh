    ...
    <div class="usa-layout padding-y-4">
      <div class="grid-row grid-gap">
        ...
        <div class="desktop:grid-col-9">
          <p>BABELMESH</p>
          <h2>MEDLINE/PubMed>の日本語による検索法</h2>
...
<font size="2">
情報を検索したい場合に自国語で検索できると大変便利です。特に英語に不慣れな方にとって翻訳された“<a href=http://www.nlm.nih.gov/pubs/factsheets/mesh.html >MeSH</a>"が使えると有用です。
</font>
<p>
<b>日本語の医学用語か関連短文を入れてください。</b></br>
</p>
...
<form  accept-charset="utf-8" method="POST" name ="jpn" id="query" action=<?php echo $_SERVER['PHP_SELF']."?com=$com"; ?>>
  ...
  <button class="g-recaptcha" 
        ...
        data-action='submit'>検索開始</button>
  <!--<input type="submit" value="検索開始" name="B1">-->
	&nbsp;<input type="reset" value="取り消し" name="B2"></p>
	<font size="2" color="#0000FF">雑誌原著の言語を選択:<br></font>
<font size="2" color="#0000FF">[<span id="s_ara_o">アラビア語</span><input type="checkbox" value="ARA" name="ARA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_chn_o">中国語</span><input type="checkbox" value="CHN" name="CHN_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_fre_o">フランス語</span><input type="checkbox" value="FRE" name="FRE_o" >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ger_o">ドイツ語</span><input type="checkbox" value="GER" name="GER_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ita_o">イタリア語</span><input type="checkbox" value="ITA" name="ITA_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_jpn_o">日本語</span><input type="checkbox" value="JPN" name="JPN_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_kor_o">韓国語</span><input type="checkbox" value="KOR" name="KOR_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_por_o">ポルトガル語</span><input type="checkbox" value="POR" name="POR_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_rus_o">ロシア語</span><input type="checkbox" value="RUS" name="RUS_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_spa_o">スペイン語</span><input type="checkbox" value="SPA" name="SPA_o"  >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_swe_o">スウェーデン語</span><input type="checkbox" value="SWE" name="SWE_o"  >]&nbsp;&nbsp;&nbsp;</font>

<font size="2" color="#0000FF">[<span id="s_eng_o">英語</span><input type="checkbox" value="ENG" name="ENG_o" >]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_all_o">全部</span><input type="checkbox" value="ALL" name="ALL_o">]</font>

	<input type = "hidden" value="1" name="checked">
</form>

<SCRIPT src="alllang.js"></SCRIPT>
<SCRIPT>InstallAC(document.jpn,document.jpn.terms,document.jpn.B1,"search","en", "jpn");;</SCRIPT>
  ...
  elseif ($_POST["checked"] == 1){
		...
		$ooterm = $_POST["terms"];
  		...
  		$db_object = db_connect($db_name);
			...
			$query = "SELECT US_DESCRIPTOR FROM jpn_mesh WHERE jpn_descriptor ='".addslashes($_POST["terms"])."'";
      ...
      $ip = get_client_ip();
	  	if ($checked == 1) {
				 ...
				 $content .= "Japanese: ".$_POST["terms"]." -- ".$rterm;
...
<form enctype="application/x-www-form-urlencoded" name="query2" action="search.php?from=jpn&com=<?php echo $com;?>&term=<?php echo $term; ?>" method="POST">
...
<?php
         ...
         mysqli_query($db_object, $query) or die("Insert query failed : ");
				   ...
				   $result = $find->lang_finder(addslashes(str_replace($de_chars, " ", $_POST["terms"]) ), $lang, $lang_table, $lang_column, $eng_column);
					 ...
					 if(sizeof($result) != 0) {
					   $FINDER = 1;
				 	   ...
				 	   $query = "SELECT JPN_DESCRIPTOR, US_DESCRIPTOR FROM jpn_mesh WHERE JPN_DESCRIPTOR like '%".addslashes($_POST["terms"])."%' GROUP BY US_DESCRIPTOR";
	  		 		 $result = mysqli_query($db_object,$query); //or die("Query failed : " );
				 		 if (mysqli_affected_rows($db_object) == 0) {
							   $result=$find->topLang(addslashes($_POST["terms"]), $lang, $lang_table, $lang_column, $eng_column);
							 	 if (mysqli_affected_rows($db_object)==0) {
							  	 echo "Sorry, this term is not in our database.<br>";
									 echo "<p><form action=search.php?from=jpn&com=$com method=POST>".$ooterm." = <input value=\"".$ooterm."\" size=25 name=terms> ";
				 ...
				 if ($FINDER == 1) {
				   $term = implode(" ", $result);		 
					 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=jpn&com=$com\" method=\"POST\">";	
				   ...
				   while ($line= mysqli_fetch_array($result)) {	
				 	   $term = $line["US_DESCRIPTOR"];
					 	 $term1 = $term;
						 $term = str_replace(' ', '+',$term);
						 $temp = $line["JPN_DESCRIPTOR"];
						 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=jpn&com=$com\" method=\"POST\">";
						 echo "<li>".str_replace($ooterm, "<B style=\"color:black;background-color:#ffff66\">".$ooterm."</B>", mb_convert_encoding($line["JPN_DESCRIPTOR"], "CP1252"));
