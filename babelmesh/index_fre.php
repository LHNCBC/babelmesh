    ...
    <div class="usa-layout padding-y-4">
      <div class="grid-row grid-gap">
        ...
        <div class="desktop:grid-col-9">
          <p>BABELMESH</p>
          <h2>Recherche MEDLINE/PubMed en français</h2>
...
<?php
  ...
  if (!array_key_exists('checked', $_POST) || $_POST["checked"] == '') {
?>

<font size="2">
En recherchant l'information, il est souvent plus facile d'utiliser le langage que l’on connait le mieux. Les traductions de 
"<a href=http://www.nlm.nih.gov/pubs/factsheets/mesh.html >MeSH</a>" 
pourraient être utiles aux personnes qui ne connaissnet pas bien l'anglais ni a fortiori le 
"<a href=http://www.nlm.nih.gov/pubs/factsheets/mesh.html >MeSH</a>" en anglais. 
Recherchez dans MEDLINE/PubMed en utilisant des expressions ou termes médicaux en français. Les enregistrements et résumés recherchés seront exclusivement en anglais. L'ajout des accents est optionnel.
</font>
<p>
<b>Écrivez le mot ou l'expression médicale</b></br>
</p>
...
<form accept-charset="utf-8" method="POST" name ="fre" id="query" action=<?php echo $_SERVER['PHP_SELF']."?com=$com"; ?>> 
  ...
  <button class="g-recaptcha" 
        ...
        data-action='submit'>Recherche</button>
  <!--<input type="submit" value="Recherche" name="B1">-->
	&nbsp;<input type="reset" value="Annuler" name="B2"></p>
<font size="2" color="#0000FF">Fouillez des articles publiés dans:<br></font>
<font size="2" color="#0000FF">[<span id="s_ara_o">Arabe</span><input type="checkbox" value="ARA" name="ARA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_chn_o">Chinois</span><input type="checkbox" value="CHN" name="CHN_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_fre_o">Français</span><input type="checkbox" value="FRE" name="FRE_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ger_o">Allemand</span><input type="checkbox" value="GER" name="GER_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_ita_o">Italien</span><input type="checkbox" value="ITA" name="ITA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_jpn_o">Japonais</span><input type="checkbox" value="JPN" name="JPN_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_kor_o">Coréen</span><input type="checkbox" value="KOR" name="KOR_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_por_o">Portugais</span><input type="checkbox" value="POR" name="POR_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_rus_o">Russe</span><input type="checkbox" value="RUS" name="RUS_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_spa_o">Espagnol</span><input type="checkbox" value="SPA" name="SPA_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_swe_o">Suédois</span><input type="checkbox" value="SWE" name="SWE_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_eng_o">Anglais</span><input type="checkbox" value="ENG" name="ENG_o">]&nbsp;&nbsp;&nbsp;</font>
<font size="2" color="#0000FF">[<span id="s_all_o">Tous</span><input type="checkbox" value="ALL" name="ALL_o">]</font>
	<input type = "hidden" value="1" name="checked">
</form>

<SCRIPT src="fr.js"></SCRIPT>
<SCRIPT>InstallAC(document.fre,document.fre.terms,document.fre.B1,"search","en");</SCRIPT>
<?php
	}
  elseif ($_POST["checked"] == 1){
			...
			if (trim($_POST["terms"]) == '') {
			  		echo "Écrivez le mot ou l'expression médical encore!";
						echo "<p><a href=".$_SERVER['PHP_SELF'].">Revenez à la page de recherche</a>";
						die();	
			}
			
			$dele= new translate();
			$_POST["terms"] = $dele->F_delete(utf8_encode($_POST["terms"]));
			$_POST["terms"] = $dele->clean_fre($_POST["terms"]);
			unset($dele);

  		$db_name = 'crosslan';
  		$db_object = db_connect($db_name);
	
			$query = "SELECT US_DESCRIPTOR FROM fre_tran WHERE FRE_DESCRIPTOR ='".addslashes($_POST["terms"])."'";
  		$result = mysqli_query($db_object,$query) or die("Query failed : " );
			if (mysqli_affected_rows($db_object) == 0) {
	  	  $query = "SELECT US_DESCRIPTOR FROM fre_tran WHERE ACCENTUATED_FRE_DESCRIPTOR ='".addslashes($_POST["terms"])."'";
  			$result = mysqli_query($db_object,$query) or die("Query failed : " );
				if (mysqli_affected_rows($db_object) == 0) {
				  $checked = 2;
				}
      ...
      $ip = get_client_ip();
	  	if ($checked == 1) {
				 $rterm = $term;
				 $term = fre_process($term);
				 $term = str_replace(' ', '+',$term);
				 $content = $ip." - [".date("j/M/Y: G:i:s")."] ";
				 $content .= "French: ".$_POST["terms"]." -- ".$rterm;
...
<form enctype="application/x-www-form-urlencoded" name="query2" action="search.php?from=fre&com=<?php echo $com;?>&term=<?php echo $term; ?>" method="POST">
...
<?php
			...
			elseif ($checked == 2) {
				 $content = $ip." - [".date("j/M/Y: G:i:s")."] ";
				 $content .= "French: ".$_POST["terms"]." -- ???";
         $query = "INSERT INTO log (search) VALUES ('$content')";
         mysqli_query($db_object, $query) or die("Insert query failed : "); 
				 $find= new translate();
				 $de_chars = array("?", ".", ",", ";", ":");
				 $result = $find->F_no_order(addslashes(str_replace($de_chars, " ", $_POST["terms"]) ));
				 if ($result == '') {
				   $result = $find->Ffinder(addslashes(str_replace($de_chars, " ", $_POST["terms"]) ));
					 $FINDER = 0;
					 if(sizeof($result) != 0) {
					   $FINDER = 1;
					 }
					 else {
				 	   $query = "SELECT FRE_DESCRIPTOR, US_DESCRIPTOR FROM fre_tran WHERE FRE_DESCRIPTOR like '".addslashes($_POST["terms"])."%' GROUP BY FRE_DESCRIPTOR";
	  		 		 $result = mysqli_query($db_object,$query) or die("Query failed : " );
				 		 if (mysqli_affected_rows($db_object) == 0) {
				 		   $query = "SELECT FRE_DESCRIPTOR, US_DESCRIPTOR FROM fre_tran WHERE FRE_DESCRIPTOR like '%".addslashes($_POST["terms"])."%' GROUP BY FRE_DESCRIPTOR";
	  		 			 $result = mysqli_query($db_object,$query); // or die("Query failed : " );
							 if (mysqli_affected_rows($db_object) == 0) {
							   $result=$find->topF(addslashes($_POST["terms"]));
							 	 if (mysqli_affected_rows($db_object)==0) {
							  	 echo "Désolé. Votre entrée n'est le mot ou l'expression médical! <br>";
									 echo "<p><form action=search.php?from=fre&com=$com method=POST>".$_POST["terms"]." = <input value=\"".$_POST["terms"]."\" size=25 name=terms> ";
									 echo "<input type=submit value=Déclic>";
				 ...
				 echo "Vous voulez dire?<p>";
				 echo "<font color=\"#3333CC\">(You may modify this search by adding or deleting English terms):</font><p>";
				 if ($FINDER == 1) {
				   $term = implode(" ", $result);
					 $term = fre_process($term);
					 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=fre&com=$com\" method=\"POST\">";	
					 $temp = $find->clean_fre($_POST["terms"]);
					 echo "<li>".$temp;
					 echo "&nbsp;=&nbsp;<input name=\"terms\" size=\"25\" value=\"".$term."\">";
					 echo "<input type = \"hidden\" name=\"Oterm\" value=\"".$temp."\">";
					 $term = str_replace(' ', '+',$term);
					 echo "&nbsp;<input type=\"submit\" value=\"Déclic\" name=\"B1\">";
					 pass_lang_out();
					 echo "</form>";
				 }
				 else {
				   while ($line= mysqli_fetch_array($result)) {
				 	   ...
				 	   $term = $line["US_DESCRIPTOR"];
						 ...
						 $term1 = $term;
						 $term = str_replace(' ', '+',$term);
						 $temp = $line["FRE_DESCRIPTOR"];
						 echo "<form enctype=\"application/x-www-form-urlencoded\" name=\"query\" action=\"search.php?from=fre&com=$com\" method=\"POST\">";
					 	 echo "<li>".str_replace(strtolower($_POST["terms"]), "<B style=\"color:black;background-color:#ffff66\">".strtolower($_POST["terms"])."</B>", strtolower($line["FRE_DESCRIPTOR"]));
						 echo "<input type = \"hidden\" name=\"Oterm\" value=\"".$temp."\">";
				 ...
				 echo "<hr><a href=".$_SERVER['PHP_SELF'].">Revenez à la page de recherche</a>";
